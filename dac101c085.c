...
void afe_offset(unsigned int code) {
  if( code > 0xFFF ) {
    printf( "Warning: code larger than 0xFFF; truncating.\n" );
    code = 0xFFF;
  }
  
  dac101c085_write_byte( (unsigned short) (code & 0xFFFF), DAC_TRIM );

#ifdef DEBUG
  unsigned short data;
  dac101c085_read_byte( &data, DAC_TRIM );
...
int main() {
  ...
  dac101c085_write_byte( 0x100, DAC_TRIM );
  dac101c085_read_byte( &data, DAC_TRIM );
  printf (" DAC101C085 readback: %03x\n", data );
  ...
  while( 1 ) {
    for( i = 0; i < 0x1000; i++ ) {
      dac101c085_write_byte( i, DAC_TRIM );
    }
    for( i = 0xFFF; i >= 0; i-- ) {
      dac101c085_write_byte( i, DAC_TRIM );
    }
